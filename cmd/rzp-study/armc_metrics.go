...
func cmdArmCReport(args []string) error {
	...
	p("# Arm C results\n\n")
	p("## Scope, and what is and is not pre-registered\n\n")
	p("A controlled scenario evaluation of the frozen guard. **Not** a\n")
	p("real-world fraud rate and **not** a model-performance claim: the corpus\n")
	p("is synthetic and mechanically enumerated, so its base rates are a\n")
	p("property of the grid, not of merchant traffic.\n\n")
	...
	p("ground-truth rule, the policy freeze, and predictions C1-C7.\n")
	p("**Amended during collection: the labelling and blinding surface.**\n")
	p("See the amendment record below.\n\n")
	p("**Generator identity is proxy-reported.** The endpoint self-reports the\n")
	p("model it served and has been measured substituting one model for\n")
	p("another, so the generator is named as requested, never as verified.\n")
	p("Nothing here is a claim about any named model; the detector under\n")
	p("evaluation is the guard.\n\n")
