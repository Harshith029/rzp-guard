...
5. **Recovery is an availability gap, not just an inconvenience.** Stop → check Razorpay → resolve → restart means a window with no guard and no forwarding service. There is **no measured recovery drill**: outage duration, what blocks new requests meanwhile, and how the operator identity is selected remain unanswered, and the CLI does not address them. Picking the correct *state file* is no longer guesswork — the file records the mandate it belongs to and refuses to open under a different one while anything is unresolved, naming what is stranded ([F22](FAILURES.md)) — but a guard rail is not a drill.
