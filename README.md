...
1. **The detector metric exists now, and it is small, model-dependent and half-bad.** Arm B measured **recall 1.000 (3/3)** and **precision 0.250 (3/12)** over 54 adjudicated calls — the guard stopped every out-of-intent refund it was shown, and three blocks in four were wrong. Three true positives come from **one** injection brief; that is not a general claim about injections, and 15 hand-written briefs adjudicated by their own author are **not a held-out sample**. Put plainly, because reviewers keep having to say it: **these are descriptive trace outcomes, not a credible held-out detector evaluation.** If asked to show the held-out positive examples that establish recall, there are three, from one brief, in one arm — and none in the other. Both arms also ran through a third-party endpoint measured substituting models, so the generator is named only as what it *self-reported*, never as verified. Arm A, against a stronger model, produced no out-of-intent calls at all, so its precision and recall are degenerate and must never be quoted as detector scores. **The arms are never pooled.** **Arm C is collected and awaiting labelling**, and until it reports, limit 1 stands exactly as written above. It is a cross product of 54 scenarios ([PROTOCOL-armC.md](study/PROTOCOL-armC.md)) the author did not select, against a policy frozen by hash beforehand, with ground truth from **external** raters who receive only a sanitised authorization-relevant projection of each call. The author's own labels are supplementary and are **not** described as blinded. When it reports it will be *an evaluation on pre-registered, model-generated controlled traces* — stronger than arm B, and **not an independent real-merchant held-out dataset**. Its grid, ground-truth rule and policy freeze are pre-registered; its labelling surface and its collection were amended mid-study ([Amendment 1](study/PROTOCOL-armC-AMENDMENT-1.md), [Amendment 2](study/PROTOCOL-armC-AMENDMENT-2.md)), and the whole labelling protocol must not be called pre-registered. Generator identity is proxy-reported. Separately, the conformance corpus in `corpus/` still cannot supply detector metrics at all: its labels are computed from the same predicate the policy matches on ([PREREGISTRATION.md Amendment 1](PREREGISTRATION.md)).
...
7. **Mandates are not authenticated. Nothing stops the operator granting authority after the fact.** `mandate.Load` parses bytes: there is no signature, no key, no ownership check, no integrity verification anywhere in `internal/mandate`. Whoever can write the mandate file grants authority — including retroactively, to cover a refund already attempted. The operator binary protects *rotation* and *resolution* with an Argon2id grant, but that is a different boundary and it does not extend to the mandate itself. So the guard's honest claim is narrower than it sounds: **it enforces that the agent cannot exceed authority someone else wrote down**, and makes every grant single-use, durable and auditable. It does not establish that the grant was legitimate. Closing this needs the mandate signed by a key the guard host does not hold — merchant-side approval, signature verified by the guard, operator unable to mint — which is a design change and not a patch. It is deliberately **not** attempted before the detector metric is finished, and is recorded here so it is found by a reader rather than by an attacker.
